...
# 注意：只用职员表/版权信息专属词，不用日常对话中可能出现的词
# 匹配规则：line.strip().startswith(kw)，避免对白中间关键词误伤
_CREDIT_KEYWORDS = [
    # 翻译/校对/时间轴
    "翻译",
    "校对",
    ...
    "统筹",
    "策划",
    # 字幕相关（精确词，不含裸"字幕"避免误伤"字幕组的大家都辛苦了"）
    "字幕组",
    "字幕制作",
    "字幕提供",
    "字幕来源",
...
def _parse_srt_entries(
    ...
    # 预处理：干掉 BOM 和空白行（仅空格/制表符的行），避免 SRT 格式偏差导致漏匹配
    # 不改变换行数量，debug 模式行号追踪保持准确
    if text.startswith("\ufeff"):
        text = text[1:]
    text = re.sub(r"\n[ \t]+\n", "\n\n", text)
...
def _find_last_content_end(
    ...
    scan_log: list[str] = []
    scan_count = min(len(entries), 100)
